...
//指令格式：ON,2|2：打开3号灯板，2级亮度  OFF,2：关闭3号灯板（指令并非来自于PC）
//没有回复
...
void TempDetect() {
  ...
  if (L1voltage > warningTemp) {
    ...
    if (L1voltage > errorTemp) {
      TurnOff(1);
      String temp = "213 1|IR lamp temp error & off";
      temp.toCharArray(sendMsg, 32);
    } else {
      String temp = "212 1|IR lamp temperature error";
      temp.toCharArray(sendMsg, 32);
    }
    L1voltage = 0;
    radio.stopListening();
  ...
  if (L2voltage > warningTemp) {
    ...
    if (L2voltage > errorTemp) {
      ...
      String temp = "212 3|IR lamp temperature error";
      temp.toCharArray(sendMsg, 32);
    }
    L2voltage = 0;
    radio.stopListening();
